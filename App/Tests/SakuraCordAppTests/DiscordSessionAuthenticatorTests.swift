...
struct DiscordSessionAuthenticatorTests {
    @Test func `cold password login stops before gateway identity and persistence`() async throws {
        ...
        #expect(AuthenticationURLProtocol.loginSuperProperties?["native_build_number"] as? Int == 89_799)
        #expect(AuthenticationURLProtocol.loginSuperProperties?["client_version"] as? String == "0.0.408")
